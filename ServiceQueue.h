	...
	void update(int clock) 
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {  
				// FIXED: remove plane from the service queue
				the_queue.pop();

				// FIXED: update the enter_departure_time attribute for the plane
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				// FIXED: calculate the wait time of the plane in the landing queue
				int wait_time = clock - plane->arrival_time;

				// FIXED: update total_wait and num_served for the landing queue
				landing_queue->total_wait += wait_time;
				landing_queue->num_served++;


				// FIXED: update the start_service_time attribute for the plane
				plane->start_service_time = clock;

				/* FIXED: compute a random service time for the plane between min_service_time and max_service_time
				...
				*/
				...
				// FIXED: add the plane to the service queue
